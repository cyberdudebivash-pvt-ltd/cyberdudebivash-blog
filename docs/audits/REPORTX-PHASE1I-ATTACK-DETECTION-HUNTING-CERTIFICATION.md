...
## 9.1 Post-review hardening (CodeRabbit, this PR)

A real automated review on this PR surfaced 4 additional, verified defects — each checked against current code before being accepted as real, per this session's standing "bot findings are bug reports, verify then fix" discipline:

1. **LLM-authored path silently dropped the hunt section** (`automation/authority_transformer.py`) — the most serious of the four. `transform()`'s LLM-success path set `body_content` to only the sanitized raw LLM HTML, never `composer_outcome.html` (which is where `hunt_html` lived), yet still passed the composer's real `hunt_hypothesis_count` to `evaluate_product_tier()`. A `cve_advisory` article published via the LLM path (the first-choice content path) could show Section 14 as `COMPLETE` while the actually-published page had zero hunt content — exactly the "claims complete when content is absent" failure this phase exists to prevent. **Fixed**: extracted a new `_render_hunt_hypotheses_html()` (mirrors `pipeline_composer.py`'s own inline renderer, operating on the dict shape `_ComposerOutcome` stores) and call it unconditionally after the Key Judgements block, guarded so it never double-renders on the `reportx_composer` path (which already includes it via `composer_outcome.html`). New test: `test_llm_authored_cve_article_still_renders_the_hunt_section_in_published_content`, against the real, unmocked `compose_report()` call with only `call_llm()` mocked.
2. **`attack_mapper.map_techniques()`'s explicit-ID citation loop checked only the first occurrence of a cited technique ID for negation** — an early negated mention ("T1219 ... was considered and rejected") could suppress a genuinely supported, non-negated citation of the same ID later in the same document. The phrase-lexicon loop in the same function already handled this correctly (checks all occurrences, takes the first non-negated one); the explicit-ID loop was inconsistent with its own sibling. Real risk given `detection_evidence_discipline` is now a hard gate: a false "no evidence" reading could incorrectly block a legitimately well-evidenced report. **Fixed**: the explicit-ID loop now checks all occurrences and only rejects a technique ID when *every* occurrence is negated. New tests: `test_explicit_id_citation_genuine_positive_after_an_earlier_negated_citation_still_maps` (positive case) and `test_explicit_id_citation_negated_everywhere_still_does_not_map` (confirms the negative case wasn't loosened).
3. **`commercial_readiness.py`'s `withheld_present` check only looked for `WITHHELD_INSUFFICIENT_EVIDENCE`, not the new `TELEMETRY_SPECIFICATION` state** — inconsistent with the sibling check `check_withheld_rules_have_rationale()`, which already treats both states as equally requiring the "explained, not just declared" discipline. A bundle whose only rules were `TELEMETRY_SPECIFICATION` (each with a real rationale) could pass `detection_evidence_discipline` with zero `intelligence_gaps` recorded. **Fixed**: `withheld_present` now checks both states. New tests: `test_telemetry_specification_rule_without_recorded_gap_fails` / `_with_recorded_gap_passes`.
4. **`T1219`'s curated label was stale** — this repo's `KNOWN_TECHNIQUES` said "Remote Access Software"; verified live against `attack.mitre.org/techniques/T1219/` that MITRE's current official name is "Remote Access Tools". **Fixed**: label corrected, `test_t1219_remote_access_software_is_curated` renamed and extended to assert the exact label string so a future edit can't silently drift again.

One finding was investigated and **deliberately not fixed**, with the reasoning left in the code rather than silently declined: `detection_validation.check_state_promotion()` scans the *entire* rendered report for promotion-language, not text scoped to the specific rule being checked — a real limitation (a phrase legitimately describing rule A could be misattributed to an unrelated rule B in the same bundle) that a full fix requires rendering-level per-rule text anchors, a genuinely separate, larger change (matches the review's own "Heavy lift" tag). Verified this is currently **unreachable** in the live pipeline: the one real call site (`pipeline_composer.compose_report()` → `_detection_rules()`) always derives every rule in a bundle from one shared `DetectionPackage.status`, so every rule in a real bundle shares one `validation_state` today. Documented explicitly in the function's own docstring, and locked in with two new tests: one demonstrating the limitation exists (`test_check_all_rules_scope_is_whole_document_not_per_rule`), one proving today's real invariant holds so a future change that breaks it fails loudly rather than silently shipping the gap (`test_the_one_real_call_site_never_produces_mixed_validation_states_in_one_bundle`).

## 10. Certification verdict

**RELEASE_CERTIFIED** for the three real changes this round makes (detection-maturity reconciliation, ATT&CK-citation hard gate, `cve_advisory` hunt-hypothesis wiring), plus the 4 post-review fixes in §9.1:

- Zero regressions across all 3 suites (1,599 tests total before post-review hardening, +11 more after, 1 pre-existing unrelated failure).
- Two real, previously-invisible defects found and fixed with minimal, verified, surgical changes during implementation — not assumed, not guessed — plus 4 more found by a real automated review and verified/fixed the same way before merge, including one (the LLM-path hunt-rendering gap) that would have made this round's own core claim (Section 14 reflects real content) false for the majority real-world content path.
...
- One real limitation (whole-document promotion-language scoping) investigated, found currently unreachable, and locked in with a tripwire test rather than silently left undocumented.
