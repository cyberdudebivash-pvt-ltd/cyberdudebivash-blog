...
describe('resolveRoute — direct api/** filesystem routes', () => {
  test('every real handler file on disk has a route (34-function parity check)', () => {
    const files = [];
    function walk(dir) {
    ...
    assert.equal(files.length, 34, 'expected exactly 34 routable api/** functions — update route-table.js if this changes');
...
describe('table sanity', () => {
  test('DIRECT_API_HANDLERS and DYNAMIC_API_HANDLERS together account for all 34 handlers with no overlap', () => {
    ...
    assert.equal(new Set(all).size, 34, 'duplicate handler path across DIRECT_API_HANDLERS/DYNAMIC_API_HANDLERS');
  });
});
