...
def evaluate_section_states(
    ...
    ``attack_mapping_count`` (RX-P1I structured ATT&CK) is len(pipeline_
    composer.ComposedReport.attack_mappings) -- every entry already passed
    ...
    for section in ALL_SECTIONS:
        ...
        if applicability == Applicability.NOT_APPLICABLE:
            ...
            state = _resolve_attack_mapping(detection_status, attack_mapping_count)
...
def _resolve_attack_mapping(detection_status: str, attack_mapping_count: int = 0) -> SectionState:
    """RX-P1I: attack_mapping_count > 0 means at least one real,
    semantic-gate-passed AttackMapping exists for this article -- checked
    first and independent of family, since map_techniques()-derived
    mappings (attack_mapping.py) are computed for every family, not only
    the CVE-vulnerability-class branches detection_status represents.
    Still never COMPLETE: every mapping this pipeline can construct is
    ASSESSED or CONDITIONAL (never OBSERVED -- see attack_mapping.py's
    module docstring for why that is structural, not a current gap), so
    PARTIAL_EVIDENCE is the honest permanent ceiling here, matching
    Section 13/21's own precedent for "real but inherently partial"
    signals (_PARTIAL_SIGNAL_ONLY). Falls back to the original
    detection_status-only check for a caller that hasn't computed
    attack_mapping_count yet (default 0) -- report_renderer.py's MITRE
    mapping line is generated by the same evidence-conditioned branch as
    the Sigma/KQL detection templates for the same vulnerability classes,
    so that fallback still means real content, never assumed."""
    if attack_mapping_count > 0:
        return SectionState.PARTIAL_EVIDENCE
